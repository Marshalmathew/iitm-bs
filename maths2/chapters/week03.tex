...
\item \textbf{Closure under Addition:} If $\mathbf{u}, \mathbf{v} \in W$, then $\mathbf{u} + \mathbf{v} \in W$.
\item \textbf{Closure under Scalar Multiplication:} If $\mathbf{v} \in W$ and $c \in \mathbb{R}$, then $c\mathbf{v} \in W$.
\end{enumerate}
\end{theorembox}
...
\begin{intuitionbox}{Degrees of Freedom \& Redundancy}
Think of vectors as giving you directions to move in space.
\begin{itemize}
    \item A \textbf{Linearly Independent} vector unlocks a brand new dimension. If you have two independent vectors in 3D space, you can travel anywhere on a flat 2D plane. A third independent vector would allow you to step off that plane and explore the entire 3D space.
    \item A \textbf{Linearly Dependent} vector is completely redundant. It's like being given a third direction that just points somewhere on the 2D plane you can already reach using the first two vectors. It provides zero new ``degrees of freedom''.
\end{itemize}
\end{intuitionbox}
...
\begin{enumerate}
    ...
    Since $1 \text{ sq ft} = 0.092903 \text{ sq meters}$, show mathematically that $\set{x_1, x_2, x_3}$ is a linearly dependent set of feature vectors, and explain why one must be removed before regression fitting.
\end{enumerate}