...
\item The leading non-zero entry (pivot) of a non-zero row is strictly to the right of the pivot in the row above it.
...
\begin{examplebox}{Solving Linear Systems in Python}
While Gaussian Elimination is vital to understand theoretically, Data Scientists never perform Row Echelon reductions by hand. We use optimized BLAS/LAPACK libraries via NumPy:
...
# np.linalg.solve uses highly optimized LU decomposition (which is just Gaussian elimination)
x = np.linalg.solve(A, b)
print(f"Solution: x = {x[0]:.1f}, y = {x[1]:.1f}")  # Solution: x = 1.0, y = 2.0
\end{lstlisting}
\end{examplebox}
