...
\textbf{1. Subspace Test for $W = \set{ [x, y]^T \mid 2x - 3y = 0 }$}
\begin{enumerate}
...
The linear combination $0.092903 \cdot x_1 - 1 \cdot x_2 + 0 \cdot x_3 = 0$ holds with non-zero coefficients. Thus the feature set $\set{x_1, x_2, x_3}$ is \textbf{linearly dependent}.
...
\begin{itemize}
    ...
    \item \textbf{(c) Similarity:} The eigenvalues of upper triangular $A$ are $\lambda_1 = 1, \lambda_2 = 3$. The eigenvalues of lower triangular $B$ are $\lambda_1 = 3, \lambda_2 = 1$. Since both have distinct eigenvalues $\set{1, 3}$, both matrices are diagonalizable to $D = \begin{bmatrix} 1 & 0 \\ 0 & 3 \end{bmatrix}$. Therefore, \textbf{$A$ and $B$ are SIMILAR} ($A \sim B$).
\end{itemize}
