...
\begin{intuitionbox}{Visualizing Probability as Area Under the Density Curve}
Below is a Gaussian curve showing how probability $P(a \le X \le b)$ corresponds to the shaded area under the curve.
\begin{center}
\begin{tikzpicture}
\begin{axis}[
...
% Density curve
\addplot[thick, primaryborder, domain=-3.5:3.5, samples=100] {exp(-x^2/2)/sqrt(2*pi)};
% Shaded probability area between -1 and 1
\addplot[fill=primarylight, draw=none, domain=-1:1, samples=100] {exp(-x^2/2)/sqrt(2*pi)} \closedcycle;
% Vertical markers
\draw[dashed] (axis cs:-1,0) -- (axis cs:-1,0.242) node[below=15pt] {$a$};
\draw[dashed] (axis cs:1,0) -- (axis cs:1,0.242) node[below=15pt] {$b$};
\node at (axis cs:0,0.1) {\small Area = $P(a \le X \le b)$};
\end{axis}
\end{tikzpicture}
\end{center}
\end{intuitionbox}
