...
\textit{Why?} $p^k$ is the probability of getting exactly $k$ successes. $(1-p)^{n-k}$ is the probability of getting exactly $n-k$ failures. We multiply by the combination $\binom{n}{k}$ to account for all the different possible orders those successes and failures could happen in.
\end{definitionbox}
