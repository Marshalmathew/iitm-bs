...
\begin{itemize}
    \item \textbf{(a)} \texttt{10 / 2}: \textbf{\texttt{float}}. In Python 3, standard division \texttt{/} always returns a floating-point number (\texttt{5.0}), even if the result is a whole number.
    \item \textbf{(b)} \texttt{10 // 2}: \textbf{\texttt{int}}. Floor division \texttt{//} truncates the fractional part and returns an integer (\texttt{5}).
    \item \textbf{(c)} \texttt{"100" + "200"}: \textbf{\texttt{str}}. Using \texttt{+} on two strings performs concatenation, producing \texttt{"100200"}.
    \item \textbf{(d)} \texttt{(5 > 3) and (2 == 4)}: \textbf{\texttt{bool}}. \texttt{5 > 3} evaluates to \texttt{True}, and \texttt{2 == 4} evaluates to \texttt{False}. \texttt{True and False} evaluates to \texttt{False}.
\end{itemize}

\textbf{2. Arithmetic Logic}
\begin{itemize}
    \item \textbf{(a)} \texttt{2 + 3 * 4 ** 2}: Exponentiation has highest precedence ($4^2 = 16$). Next is multiplication ($3 \times 16 = 48$). Finally addition ($2 + 48 = \mathbf{50}$).
    \item \textbf{(b)} \texttt{25 \% 7}: Modulus gives the remainder of $25 \div 7$. $7 \times 3 = 21$, leaving a remainder of $\mathbf{4}$.
    \item \textbf{(c)} \texttt{19 // 4}: Floor division of $19 \div 4 = 4.75$, dropping the decimal gives $\mathbf{4}$.
\end{itemize}

\textbf{3. String Slicing}
\begin{itemize}
    \item \textbf{(a)} \texttt{text[0:7]} or \texttt{text[:7]} returns \texttt{"Machine"}.
    \item \textbf{(b)} \texttt{text[7:]} returns \texttt{"Learning"}.
    \item \textbf{(c)} \texttt{text[-8:]} extracts the last 8 characters, which is \texttt{"Learning"}.
    \item \textbf{(d)} \texttt{text[::-1]} reverses the string to \texttt{"gninraeLEnihcaM"}.
\end{itemize}
...
The output is \textbf{\texttt{Safe}}.
Because \texttt{x = 0}, the first operand \texttt{x != 0} evaluates to \texttt{False}. Due to short-circuit evaluation, Python immediately knows the entire \texttt{and} expression is \texttt{False} without evaluating the second term \texttt{(10 / x > 2)}. Thus, division by zero never occurs, preventing a runtime crash.
...
\textbf{4. Early Stopping (\texttt{break})}
\begin{lstlisting}
...
\textbf{3. Flexible Aggregation (\texttt{*args})}
\begin{lstlisting}
...
# Outside: 50
\end{lstlisting}
\textbf{Explanation:} Inside \texttt{modify\_val()}, the assignment \texttt{x = 100} creates a new \emph{local} variable \texttt{x} within the function scope. It shadows the global \texttt{x} without modifying it. Once the function finishes, the local variable is destroyed, and the global variable \texttt{x} remains \texttt{50}.
...
Array: \texttt{[2, 5, 8, 12, 16, 23, 38, 56, 72, 91]}, Target: \texttt{23}.
\begin{enumerate}
    \item \textbf{Step 1:} \texttt{low = 0}, \texttt{high = 9}. \texttt{mid = (0 + 9) // 2 = 4}. \texttt{arr[4] = 16}. Since $23 > 16$, search right half: \texttt{low = 5}.
    \item \textbf{Step 2:} \texttt{low = 5}, \texttt{high = 9}. \texttt{mid = (5 + 9) // 2 = 7}. \texttt{arr[7] = 56}. Since $23 < 56$, search left half: \texttt{high = 6}.
    \item \textbf{Step 3:} \texttt{low = 5}, \texttt{high = 6}. \texttt{mid = (5 + 6) // 2 = 5}. \texttt{arr[5] = 23}. Match found!
\end{enumerate}
