...
\begin{itemize}
    \item \textbf{(a)} \lstinline|10 / 2|: \textbf{\lstinline|float|}. In Python 3, standard division \lstinline|/| always returns a floating-point number (\lstinline|5.0|), even if the result is a whole number.
    \item \textbf{(b)} \lstinline|10 // 2|: \textbf{\lstinline|int|}. Floor division \lstinline|//| truncates the fractional part and returns an integer (\lstinline|5|).
    \item \textbf{(c)} \lstinline|"100" + "200"|: \textbf{\lstinline|str|}. Using \lstinline|+| on two strings performs concatenation, producing \lstinline|"100200"|.
    \item \textbf{(d)} \lstinline|(5 > 3) and (2 == 4)|: \textbf{\lstinline|bool|}. \lstinline|5 > 3| evaluates to \lstinline|True|, and \lstinline|2 == 4| evaluates to \lstinline|False|. \lstinline|True and False| evaluates to \lstinline|False|.
\end{itemize}

\textbf{2. Arithmetic Logic}
\begin{itemize}
    \item \textbf{(a)} \lstinline|2 + 3 * 4 ** 2|: Exponentiation has highest precedence ($4^2 = 16$). Next is multiplication ($3 \times 16 = 48$). Finally addition ($2 + 48 = \mathbf{50}$).
    \item \textbf{(b)} \lstinline|25 % 7|: Modulus gives the remainder of $25 \div 7$. $7 \times 3 = 21$, leaving a remainder of $\mathbf{4}$.
    \item \textbf{(c)} \lstinline|19 // 4|: Floor division of $19 \div 4 = 4.75$, dropping the decimal gives $\mathbf{4}$.
\end{itemize}

\textbf{3. String Slicing}
\begin{itemize}
    \item \textbf{(a)} \lstinline|text[0:7]| or \lstinline|text[:7]| returns \lstinline|"Machine"|.
    \item \textbf{(b)} \lstinline|text[7:]| returns \lstinline|"Learning"|.
    \item \textbf{(c)} \lstinline|text[-8:]| extracts the last 8 characters, which is \lstinline|"Learning"|.
    \item \textbf{(d)} \lstinline|text[::-1]| reverses the string to \lstinline|"gninraeLEnihcaM"|.
\end{itemize}
...
The output is \textbf{\lstinline|Safe|}.
Because \lstinline|x = 0|, the first operand \lstinline|x != 0| evaluates to \lstinline|False|. Due to short-circuit evaluation, Python immediately knows the entire \lstinline|and| expression is \lstinline|False| without evaluating the second term \lstinline|(10 / x > 2)|. Thus, division by zero never occurs, preventing a runtime crash.
...
\textbf{4. Early Stopping (\lstinline|break|)}
\begin{lstlisting}
...
\textbf{3. Flexible Aggregation (\lstinline|*args|)}
\begin{lstlisting}
...
# Outside: 50
\end{lstlisting}
\textbf{Explanation:} Inside \lstinline|modify_val()|, the assignment \lstinline|x = 100| creates a new \emph{local} variable \lstinline|x| within the function scope. It shadows the global \lstinline|x| without modifying it. Once the function finishes, the local variable is destroyed, and the global variable \lstinline|x| remains \lstinline|50|.
...
Array: \lstinline|[2, 5, 8, 12, 16, 23, 38, 56, 72, 91]|, Target: \lstinline|23|.
\begin{enumerate}
    \item \textbf{Step 1:} \lstinline|low = 0|, \lstinline|high = 9|. \lstinline|mid = (0 + 9) // 2 = 4|. \lstinline|arr[4] = 16|. Since $23 > 16$, search right half: \lstinline|low = 5|.
    \item \textbf{Step 2:} \lstinline|low = 5|, \lstinline|high = 9|. \lstinline|mid = (5 + 9) // 2 = 7|. \lstinline|arr[7] = 56|. Since $23 < 56$, search left half: \lstinline|high = 6|.
    \item \textbf{Step 3:} \lstinline|low = 5|, \lstinline|high = 6|. \lstinline|mid = (5 + 6) // 2 = 5|. \lstinline|arr[5] = 23|. Match found!
\end{enumerate}
