...
A variable is created the moment you first assign a value to it using the assignment operator (\lstinline|=|).
\begin{lstlisting}
...
\begin{itemize}
    \item \textbf{Integer (\lstinline|int|):} Whole numbers without decimals (e.g., \lstinline|-5|, \lstinline|0|, \lstinline|42|).
    \item \textbf{Float (\lstinline|float|):} Real numbers with decimal points or scientific notation (e.g., \lstinline|3.14|, \lstinline|-0.001|, \lstinline|2.5e-4|).
    \item \textbf{String (\lstinline|str|):} Sequences of text characters enclosed in single, double, or triple quotes (e.g., \lstinline|"data"|, \lstinline|'Python'|).
    \item \textbf{Boolean (\lstinline|bool|):} Truth values: \lstinline|True| or \lstinline|False|.
\end{itemize}
You can inspect the type of any variable using the built-in \lstinline|type()| function.
\end{definitionbox}
...
Strings are immutable sequences of characters. You can access individual characters using 0-based indexing or slice sub-strings using the bracket syntax \lstinline|[start:stop:step]|.

\begin{formulabox}{String Slicing Syntax}
Given \lstinline|s = "DataScience"|:
\begin{itemize}
    \item \lstinline|s[0]| $\to$ \lstinline|'D'| (First character)
    \item \lstinline|s[-1]| $\to$ \lstinline|'e'| (Last character via negative indexing)
    \item \lstinline|s[0:4]| $\to$ \lstinline|'Data'| (Slice from index 0 up to, but not including, index 4)
    \item \lstinline|s[::-1]| $\to$ \lstinline|'ecneicSataD'| (Reverse the string)
\end{itemize}
\end{formulabox}
...
\begin{itemize}
    \item \textbf{Arithmetic:} \lstinline|+| (add), \lstinline|-| (subtract), \lstinline|*| (multiply), \lstinline|/| (float division), \lstinline|//| (floor division), \lstinline|%| (modulus / remainder), \lstinline|\textbf{| (exponentiation).
    \item \textbf{Comparison:} \lstinline|==| (equal), \lstinline|!=| (not equal), \lstinline|>|, \lstinline|<|, \lstinline|>=|, \lstinline|<=|.
    \item \textbf{Logical:} \lstinline|and|, \lstinline|or|, \lstinline|not|.
\end{itemize}
\end{theorembox}
...
\begin{enumerate}
    \item \textbf{Variable Types:} Identify the data type (\lstinline|int|, \lstinline|float|, \lstinline|str|, or \lstinline|bool|) of the following Python expressions:
    \begin{enumerate}
        \item \lstinline|x = 10 / 2|
        \item \lstinline|y = 10 // 2|
        \item \lstinline|z = "100" + "200"|
        \item \lstinline|w = (5 > 3) and (2 == 4)|
    \end{enumerate}
    ...
    \begin{enumerate}
        \item \lstinline|2 + 3 * 4 ** 2|
        \item \lstinline|25 % 7|
        \item \lstinline|19 // 4|
    \end{enumerate}

    \item \textbf{String Slicing:} Given the string \lstinline|text = "MachineLearning"|:
    \begin{enumerate}
        \item Write a slice to extract \lstinline|"Machine"|.
        \item Write a slice to extract \lstinline|"Learning"|.
        \item What is the output of \lstinline|text[-8:]|?
        \item Write a slice to reverse the entire string.
    \end{enumerate}

    \item \textbf{Type Casting \& Input:} A user enters their age as a string \lstinline|"22"|. Write a 2-line Python script that converts this input into an integer, adds $5$ years, and prints: \lstinline|"In 5 years, you will be 27"|.
    ...
    Given \lstinline|weight = 75| and \lstinline|height = 1.75|, write a Python snippet to compute the BMI and round it to 2 decimal places using \lstinline|round(bmi, 2)|.
\end{enumerate}