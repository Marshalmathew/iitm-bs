...
A variable is created the moment you first assign a value to it using the assignment operator (\texttt{=}).
\begin{lstlisting}
...
\begin{itemize}
    \item \textbf{Integer (\texttt{int}):} Whole numbers without decimals (e.g., \texttt{-5}, \texttt{0}, \texttt{42}).
    \item \textbf{Float (\texttt{float}):} Real numbers with decimal points or scientific notation (e.g., \texttt{3.14}, \texttt{-0.001}, \texttt{2.5e-4}).
    \item \textbf{String (\texttt{str}):} Sequences of text characters enclosed in single, double, or triple quotes (e.g., \texttt{"data"}, \texttt{'Python'}).
    \item \textbf{Boolean (\texttt{bool}):} Truth values: \texttt{True} or \texttt{False}.
\end{itemize}
You can inspect the type of any variable using the built-in \texttt{type()} function.
\end{definitionbox}
...
Strings are immutable sequences of characters. You can access individual characters using 0-based indexing or slice sub-strings using the bracket syntax \texttt{[start:stop:step]}.

\begin{formulabox}{String Slicing Syntax}
Given \texttt{s = "DataScience"}:
\begin{itemize}
    \item \texttt{s[0]} $\to$ \texttt{'D'} (First character)
    \item \texttt{s[-1]} $\to$ \texttt{'e'} (Last character via negative indexing)
    \item \texttt{s[0:4]} $\to$ \texttt{'Data'} (Slice from index 0 up to, but not including, index 4)
    \item \texttt{s[::-1]} $\to$ \texttt{'ecneicSataD'} (Reverse the string)
\end{itemize}
\end{formulabox}
...
\begin{itemize}
    \item \textbf{Arithmetic:} \texttt{+} (add), \texttt{-} (subtract), \texttt{*} (multiply), \texttt{/} (float division), \texttt{//} (floor division), \texttt{\%} (modulus / remainder), \texttt{\textbackslash\{\}textbf\{} (exponentiation).
    \item \textbf{Comparison:} \texttt{==} (equal), \texttt{!=} (not equal), \texttt{>}, \texttt{<}, \texttt{>=}, \texttt{<=}.
...
\begin{enumerate}
    \item \textbf{Variable Types:} Identify the data type (\texttt{int}, \texttt{float}, \texttt{str}, or \texttt{bool}) of the following Python expressions:
    \begin{enumerate}
        \item \texttt{x = 10 / 2}
        \item \texttt{y = 10 // 2}
        \item \texttt{z = "100" + "200"}
        \item \texttt{w = (5 > 3) and (2 == 4)}
    \end{enumerate}
    ...
    \item \textbf{String Slicing:} Given the string \texttt{text = "MachineLearning"}:
    \begin{enumerate}
        \item Write a slice to extract \texttt{"Machine"}.
        \item Write a slice to extract \texttt{"Learning"}.
        \item What is the output of \texttt{text[-8:]}?
        \item Write a slice to reverse the entire string.
    \end{enumerate}

    \item \textbf{Type Casting \& Input:} A user enters their age as a string \texttt{"22"}. Write a 2-line Python script that converts this input into an integer, adds $5$ years, and prints: \texttt{"In 5 years, you will be 27"}.
    ...
    Given \texttt{weight = 75} and \texttt{height = 1.75}, write a Python snippet to compute the BMI and round it to 2 decimal places using \texttt{round(bmi, 2)}.
\end{enumerate}