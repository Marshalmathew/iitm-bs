...
A \textbf{Tuple} is an ordered, immutable sequence of elements defined using parentheses `()`.
...
\textbf{Data Science Relevance:} Tuples guarantee data integrity for fixed coordinates, dataset shapes (`df.shape` returns `(rows, cols)`), and function return values.
...
A \textbf{Dictionary} store elements in `{key: value}` pairs. Dictionaries provide $\mathcal{O}(1)$ average-time complexity for retrieval by key.
...
Official lecture detailing dictionary creation, `.keys()`, `.values()`, `.items()`, and `.get()` methods.
\end{videocard}

\section{Sets: Unordered Unique Collections}

A \textbf{Set} is an unordered collection of unique elements defined using `{}` or `set()`.

\begin{formulabox}{Set Operations}
\begin{itemize}
    \item \textbf{Union (`A | B`):} Elements in $A$, $B$, or both.
    \item \textbf{Intersection (`A \& B`):} Elements in both $A$ and $B$.
    \item \textbf{Difference (`A - B`):} Elements in $A$ but not in $B$.
    \item \textbf{Symmetric Difference (`A \textasciicircum{} B`):} Elements in $A$ or $B$ but not both.
\end{itemize}
\end{formulabox}
...
\begin{enumerate}
    \item \textbf{Tuple Operations:} Given `student = ("Alice", 21, "Data Science", 3.8)`:
    \begin{enumerate}
        \item Unpack this tuple into variables `name`, `age`, `major`, and `gpa`.
        \item What happens if you try to modify `student[1] = 22`?
    \end{enumerate}

    \item \textbf{Dictionary Lookup \& Updating:} Create a dictionary `stock_prices = {"AAPL": 175.5, "GOOGL": 140.2, "MSFT": 380.0}`:
    \begin{enumerate}
        \item Update the price of `"AAPL"` to `180.0`.
        \item Add a new key `"AMZN"` with price `150.0`.
        \item Safely fetch the price of `"NVDA"` using `.get()` without raising a `KeyError`, returning `0.0` if missing.
    \end{enumerate}

    \item \textbf{Set Membership \& Deduplication:} Given a raw customer ID list with duplicates `ids = [101, 102, 101, 105, 102, 108]`:
    \begin{enumerate}
        \item Convert the list to a set to remove all duplicates.
        \item Check if customer `105` is in the set.
    \end{enumerate}
    ...
    Given `labels = ["cat", "dog", "cat", "bird", "dog"]`, write a Python snippet to extract unique labels into a dictionary `label2idx` mapping each unique label to a unique integer index (e.g. `{"cat": 0, "dog": 1, "bird": 2}`).
\end{enumerate}