...
A \textbf{Tuple} is an ordered, immutable sequence of elements defined using parentheses \texttt{()}.
...
\textbf{Data Science Relevance:} Tuples guarantee data integrity for fixed coordinates, dataset shapes (\texttt{df.shape} returns \texttt{(rows, cols)}), and function return values.
...
A \textbf{Dictionary} store elements in \texttt{\{key: value\}} pairs. Dictionaries provide $\mathcal{O}(1)$ average-time complexity for retrieval by key.
...
Official lecture detailing dictionary creation, \texttt{.keys()}, \texttt{.values()}, \texttt{.items()}, and \texttt{.get()} methods.
\end{videocard}

\section{Sets: Unordered Unique Collections}

A \textbf{Set} is an unordered collection of unique elements defined using \texttt{\{\}} or \texttt{set()}.

\begin{formulabox}{Set Operations}
\begin{itemize}
    \item \textbf{Union (\texttt{A } B|):} Elements in $A$, $B$, or both.
    \item \textbf{Intersection (\texttt{A \textbackslash\{\}\& B}):} Elements in both $A$ and $B$.
    \item \textbf{Difference (\texttt{A - B}):} Elements in $A$ but not in $B$.
    \item \textbf{Symmetric Difference (\texttt{A \textbackslash\{\}textasciicircum\{\} B}):} Elements in $A$ or $B$ but not both.
\end{itemize}
\end{formulabox}
...
\begin{enumerate}
    \item \textbf{Tuple Operations:} Given \texttt{student = ("Alice", 21, "Data Science", 3.8)}:
    \begin{enumerate}
        \item Unpack this tuple into variables \texttt{name}, \texttt{age}, \texttt{major}, and \texttt{gpa}.
        \item What happens if you try to modify \texttt{student[1] = 22}?
    \end{enumerate}

    \item \textbf{Dictionary Lookup \& Updating:} Create a dictionary \texttt{stock\_prices = \{"AAPL": 175.5, "GOOGL": 140.2, "MSFT": 380.0\}}:
    \begin{enumerate}
        \item Update the price of \texttt{"AAPL"} to \texttt{180.0}.
        \item Add a new key \texttt{"AMZN"} with price \texttt{150.0}.
        \item Safely fetch the price of \texttt{"NVDA"} using \texttt{.get()} without raising a \texttt{KeyError}, returning \texttt{0.0} if missing.
    \end{enumerate}

    \item \textbf{Set Membership \& Deduplication:} Given a raw customer ID list with duplicates \texttt{ids = [101, 102, 101, 105, 102, 108]}:
    \begin{enumerate}
        \item Convert the list to a set to remove all duplicates.
        \item Check if customer \texttt{105} is in the set.
    \end{enumerate}
    ...
    Given \texttt{labels = ["cat", "dog", "cat", "bird", "dog"]}, write a Python snippet to extract unique labels into a dictionary \texttt{label2idx} mapping each unique label to a unique integer index (e.g. \texttt{\{"cat": 0, "dog": 1, "bird": 2\}}).
\end{enumerate}