...
\textbf{NumPy} (Numerical Python) introduces the `ndarray` (N-dimensional array), an array of homogeneous elements stored contiguously in memory.

\begin{definitionbox}{NumPy Arrays and Vectorization}
\textbf{Vectorization} allows mathematical operations to operate on entire arrays simultaneously without explicit Python `for` loops.
\begin{lstlisting}
import numpy as np
...
\textbf{Data Science Relevance:} Pandas is the default tool for Exploratory Data Analysis (EDA), CSV/Parquet loading (`pd.read_csv()`), handling missing values (`df.fillna()`), and feature engineering.
...
Official lecture detailing indexing, row selection (`.loc`, `.iloc`), and column operations.
\end{videocard}
...
\begin{enumerate}
    \item \textbf{NumPy Vectorization:} Given `arr = np.arange(1, 11)` (integers $1$ to $10$):
    \begin{enumerate}
        \item Compute a boolean mask for all odd numbers.
        \item Use the boolean mask to extract only the odd elements.
        \item Replace all even numbers in `arr` with $-1$.
    \end{enumerate}

    \item \textbf{Matrix Statistics with NumPy:} Create a $3 \times 3$ matrix with values $1$ through $9$ using `np.arange(1, 10).reshape(3, 3)`. Calculate:
    \begin{enumerate}
        \item Sum of all elements in the matrix.
        \item Column-wise means (`axis=0`).
        \item Row-wise maximums (`axis=1`).
    \end{enumerate}
...
\end{lstlisting}
    Filter and print rows where `Temp > 30` AND `Humidity > 70`.

    \item \textbf{Pandas Missing Data Handling:} Create a DataFrame with missing values `np.nan`:
    `df = pd.DataFrame({'A': [1.0, 2.0, np.nan, 4.0], 'B': [np.nan, 10.0, 20.0, 30.0]})`.
    Fill missing values in column `'A'` with its mean, and column `'B'` with `0.0`.
...
\end{lstlisting}
    Write a 1-line Pandas expression using `.groupby()` to calculate the total revenue per category.
\end{enumerate}