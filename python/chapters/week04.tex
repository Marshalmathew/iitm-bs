...
Official lecture computing vector dot products using `for` loops.
\end{videocard}
...
\begin{enumerate}
    \item \textbf{List Operations:} Start with `data = [15, 8, 42, 4, 23]`:
    \begin{enumerate}
        \item Append $16$ to the list.
        \item Sort the list in ascending order using `.sort()`.
    ...
    \item \textbf{Matrix Transpose:} Write a Python script using nested `for` loops to compute the transpose $A^T$ of a $2 \times 3$ matrix:
    $$A = \begin{bmatrix} 1 & 2 & 3 \\ 4 & 5 & 6 \end{bmatrix} \implies A^T = \begin{bmatrix} 1 & 4 \\ 2 & 5 \\ 3 & 6 \end{bmatrix}$$

    \item \textbf{Manual Selection Sort:} Implement the selection sort algorithm in Python to sort `arr = [64, 25, 12, 22, 11]` without using built-in `.sort()` or `sorted()`.
    ...
    Given `features = [10, 20, 30, 40, 50]`, write a `for` loop to compute the min-max normalized list.
\end{enumerate}