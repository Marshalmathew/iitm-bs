...
1. \textbf{Base Case:} The stopping condition that returns a value directly without further recursive calls (prevents \texttt{RecursionError: maximum recursion depth exceeded}).
...
Compare the target with the middle element \texttt{mid}:
\begin{itemize}
    \item If \texttt{target == arr[mid]}, return \texttt{mid}.
    \item If \texttt{target < arr[mid]}, search the left sub-array (\texttt{high = mid - 1}).
    \item If \texttt{target > arr[mid]}, search the right sub-array (\texttt{low = mid + 1}).
\end{itemize}
\end{theorembox}
...
\begin{enumerate}
    \item \textbf{Recursive Sum:} Write a recursive function \texttt{recursive\_sum(n)} that calculates the sum of integers from $1$ to $n$.

    \item \textbf{Recursive String Reversal:} Write a recursive function \texttt{reverse\_string(s)} that reverses a string without using loops or slicing \texttt{[::-1]}. (Base case: empty string or length 1).

    \item \textbf{Binary Search Trace:} Given a sorted array \texttt{arr = [2, 5, 8, 12, 16, 23, 38, 56, 72, 91]}:
    \begin{enumerate}
        \item Trace the values of \texttt{low}, \texttt{high}, and \texttt{mid} when searching for \texttt{target = 23}.
        \item How many comparison steps were required?
    \end{enumerate}

    \item \textbf{Recursive Fibonacci with Memoization:} The naïve recursive Fibonacci $F(n) = F(n-1) + F(n-2)$ takes $\mathcal{O}(2^n)$ exponential time. Write an optimized version \texttt{fib\_memo(n, memo=\{\})} that uses a dictionary to cache previously computed Fibonacci numbers in $\mathcal{O}(n)$ time.

    \item \textbf{Data Science Application (Finding Square Root via Binary Search):} Write a function \texttt{sqrt\_binary\_search(val, tol=1e-5)} that uses binary search on the interval $[0, \text{val}]$ to find the square root of a non-negative float \texttt{val} within tolerance \texttt{tol}.
\end{enumerate}