...
1. \textbf{Base Case:} The stopping condition that returns a value directly without further recursive calls (prevents \lstinline|RecursionError: maximum recursion depth exceeded|).
...
Compare the target with the middle element \lstinline|mid|:
\begin{itemize}
    \item If \lstinline|target == arr[mid]|, return \lstinline|mid|.
    \item If \lstinline|target < arr[mid]|, search the left sub-array (\lstinline|high = mid - 1|).
    \item If \lstinline|target > arr[mid]|, search the right sub-array (\lstinline|low = mid + 1|).
\end{itemize}
\end{theorembox}
...
\begin{enumerate}
    \item \textbf{Recursive Sum:} Write a recursive function \lstinline|recursive_sum(n)| that calculates the sum of integers from $1$ to $n$.

    \item \textbf{Recursive String Reversal:} Write a recursive function \lstinline|reverse_string(s)| that reverses a string without using loops or slicing \lstinline|[::-1]|. (Base case: empty string or length 1).

    \item \textbf{Binary Search Trace:} Given a sorted array \lstinline|arr = [2, 5, 8, 12, 16, 23, 38, 56, 72, 91]|:
    \begin{enumerate}
        \item Trace the values of \lstinline|low|, \lstinline|high|, and \lstinline|mid| when searching for \lstinline|target = 23|.
        \item How many comparison steps were required?
    \end{enumerate}

    \item \textbf{Recursive Fibonacci with Memoization:} The naïve recursive Fibonacci $F(n) = F(n-1) + F(n-2)$ takes $\mathcal{O}(2^n)$ exponential time. Write an optimized version \lstinline|fib_memo(n, memo={})| that uses a dictionary to cache previously computed Fibonacci numbers in $\mathcal{O}(n)$ time.

    \item \textbf{Data Science Application (Finding Square Root via Binary Search):} Write a function \lstinline|sqrt_binary_search(val, tol=1e-5)| that uses binary search on the interval $[0, \text{val}]$ to find the square root of a non-negative float \lstinline|val| within tolerance \lstinline|tol|.
\end{enumerate}