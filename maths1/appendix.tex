...
\textbf{1. Pure Math: Let $A = \{1, 2, 3\}$ and $B = \{3, 4, 5\}$. Find $(A \cup B) \setminus (A \cap B)$.}
\begin{itemize}
    \item First, find the union $A \cup B$, which represents all elements in either set: $A \cup B = \{1, 2, 3, 4, 5\}$.
    \item Next, find the intersection $A \cap B$, which represents elements present in both sets: $A \cap B = \{3\}$.
    \item Finally, take the difference, which removes elements of the intersection from the union: $(A \cup B) \setminus (A \cap B) = \{1, 2, 4, 5\}$.
\end{itemize}
