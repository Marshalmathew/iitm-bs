...
\begin{itemize}
    \item \textbf{Natural Numbers ($\N$):} $\N = \set{1, 2, 3, \dots}$. These are counting numbers. In a dataset, the number of clicks on a website or the number of words in an email would be represented using natural numbers. (Note: Some contexts include $0$).
    \item \textbf{Integers ($\Z$):} $\Z = \set{\dots, -2, -1, 0, 1, 2, \dots}$. Integers include negative values. We often use integers to represent changes in stock prices (up or down by whole amounts) or differences in counts over time.
...
\begin{itemize}
    ...
    \item \index{Cartesian Product}\textbf{Cartesian Product ($A \times B$):} The set of all possible ordered pairs where the first element is from $A$ and the second is from $B$: $\set{(a, b) \mid a \in A, b \in B}$.
\end{itemize}
...
\centering
\begin{venndiagram2sets}[tikzoptions={scale=0.6, transform shape}]
\fillA \fillB
...
\fillNotA
\end{venndiagram2sets}\\
\textbf{Complement} $A^c$
\end{minipage}
\vspace{0.5cm}

\begin{examplebox}{Set Operations in Python}
In Data Science, we routinely perform these exact mathematical operations using Python's \texttt{set} object:
...
Find the power set of $S = \set{a, b, c}$.
\begin{itemize}
    \item Since $|S| = 3$, the power set must contain $2^3 = 8$ elements.
    \item The subsets are:
    \begin{align*}
        \text{0-element subsets:} & \quad \emptyset \\
        \text{1-element subsets:} & \quad \set{a}, \set{b}, \set{c} \\
        \text{2-element subsets:} & \quad \set{a,b}, \set{b,c}, \set{a,c} \\
        \text{3-element subsets:} & \quad \set{a,b,c}
    \end{align*}
    \item Thus, the power set is:
    $$\mathcal{P}(S) = \Big\{ \emptyset, \set{a}, \set{b}, \set{c}, \set{a,b}, \set{b,c}, \set{a,c}, \set{a,b,c} \Big\}$$
\end{itemize}
\end{examplebox}
...
\begin{intuitionbox}{Relations as Database Joins}
Mathematically, saying a relation is a "subset of a Cartesian product" sounds abstract. Think of it visually like a table in a database:
\begin{itemize}
    \item Let $A$ be a set of Students, and $B$ be a set of Courses.
    \item The Cartesian product $A \times B$ generates every possible combination of every student with every course.
    \item A \textbf{Relation} $R$ (e.g., "Enrolled In") simply filters out the nonsense. It picks out only the specific $(Student, Course)$ pairs that actually exist in the real world.
\end{itemize}
When you perform an \texttt{INNER JOIN} in SQL, you are quite literally computing a mathematical relation!
\end{intuitionbox}
...
\begin{itemize}
    ...
    \item \index{Range}\textbf{Range/Image:} The set of actual outputs produced by the function: $\text{Im}(f) = \set{ f(x) \mid x \in A }$. Note that the range is a subset of the codomain ($\text{Im}(f) \subseteq B$).
...
\begin{intuitionbox}{Visualizing Injectivity: The Horizontal Line Test}
A quick way to determine if a function $\func{f}{\R}{\R}$ is injective is to graph it. Since an injective function can only have one unique $x$ input for any $y$ output, \textbf{no horizontal line can intersect the graph more than once}.
\begin{center}
\begin{tikzpicture}
\begin{axis}[
    ...
    xlabel={$x$}, ylabel={$f(x)$},
    ticks=none,
    title={Not Injective: $f(x) = x^2$}
]
\addplot[blue, thick, domain=-2.8:2.8, samples=50] {x^2};
\addplot[red, thick, dashed, domain=-3:3] {4};
\node[red, above] at (axis cs: 1.5, 4) {Horizontal Line $y=4$};
\fill[red] (axis cs: 2, 4) circle (2pt) node[right, black] {$(2, 4)$};
\fill[red] (axis cs: -2, 4) circle (2pt) node[left, black] {$(-2, 4)$};
\end{axis}
\end{tikzpicture}
\end{center}
Notice how $f(2) = 4$ and $f(-2) = 4$. Two distinct inputs map to the exact same output, violating injectivity!
\end{intuitionbox}
...
\begin{enumerate}
    \item \textbf{Pure Math:} Let $A = \set{1, 2, 3}$ and $B = \set{3, 4, 5}$. Find $(A \cup B) \setminus (A \cap B)$. This operation is known as the symmetric difference.
