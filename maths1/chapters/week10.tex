...
\begin{itemize}
    \item \textbf{Undirected Graph:} Edges have no orientation. An edge is an unordered pair $\{u, v\}$.
...
Consider a simple graph with vertices $\{0, 1, 2, 3\}$ and edges $\{(0,1), (0,2), (1,2), (2,3)\}$. Let's trace BFS starting at node $0$.
...
\hline
2 & Pop $0$ & $[\,]$ & $\{0\}$ \\
  & Push neighbors $1, 2$ & $[1, 2]$ & $\{0, 1, 2\}$ \\
\hline
3 & Pop $1$ & $[2]$ & $\{0, 1, 2\}$ \\
  & Neighbors already visited & $[2]$ & $\{0, 1, 2\}$ \\
\hline
4 & Pop $2$ & $[\,]$ & $\{0, 1, 2\}$ \\
  & Push neighbor $3$ & $[3]$ & $\{0, 1, 2, 3\}$ \\
\hline
5 & Pop $3$ & $[\,]$ & $\{0, 1, 2, 3\}$ \\
  & Neighbors already visited & $[\,]$ & $\{0, 1, 2, 3\}$ \\
\hline
\end{tabular}
\end{center}
