...
\begin{examplebox}{NetworkX Implementation: Dijkstra \& Kruskal MST}
In Data Science, graph algorithms are efficiently executed using the \texttt{networkx} Python library:
...
length = nx.dijkstra_path_length(G, source='A', target='E')
print(f"Shortest path A -> E: {path} with weight {length}")
