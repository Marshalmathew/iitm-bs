...
Warshall's algorithm is a dynamic programming approach that constructs a boolean reachability matrix $R^{(k)}$ where $R^{(k)}_{ij} = 1$ if there is a path from $i \to j$ using only intermediate vertices from the set $\{1, 2, \dots, k\}$.
\begin{itemize}
